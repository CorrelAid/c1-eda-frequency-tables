...
```{r}
question_id <- "q1"
res <- dbSendQuery(con, sprintf("SELECT value, COUNT(value) FROM q_response_labelled_de_v1 WHERE question_item_id = '%s' GROUP BY value ", question_id))
dbFetch(res)
```
...
```{r}
question_id <- "q28"
res <- dbSendQuery(con, sprintf("SELECT subquestion_id, value, COUNT(value) FROM q_response_labelled_en_v1 WHERE  question_item_id IN ( SELECT question_item_id from q_response_labelled_en_v1 where question_item_id = '%s') GROUP BY value, subquestion_id", question_id))
dbFetch(res)
```
